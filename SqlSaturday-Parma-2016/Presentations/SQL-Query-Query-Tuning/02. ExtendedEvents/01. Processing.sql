...
SELECT 
	...
	x.event_data
FROM 
	sys.fn_xe_file_target_read_file ('C:\Demos\Sessions\query_performance*.xel', 'C:\Demos\Sessions\query_performance*.xem', null, null)
CROSS APPLY 
	(SELECT CAST(event_data AS XML) AS event_data) as x ;
...
WHERE 
	x.query_hash > 0
	AND (x.event_type = 'sp_statement_completed' 
	OR x.event_type = 'sql_statement_completed');
